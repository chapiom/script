...
sh_ver="1.4.0"
github="gh-proxy.com/raw.githubusercontent.com/chiakge/Linux-NetSpeed/master"
...
installbbr(){
	...
	if [[ "${release}" == "centos" ]]; then
		...
		mkdir bbr && cd bbr
		wget http://mirrors.ustc.edu.cn/debian-security/pool/updates/main/o/openssl/libssl1.1_1.1.1n-0+deb10u6_amd64.deb
...
startbbrmod_nanqinlang(){
	...
	if [[ "${release}" == "centos" ]]; then
		yum install -y make gcc
		mkdir bbrmod && cd bbrmod
		wget -N --no-check-certificate https://gh-proxy.com/raw.githubusercontent.com/chiakge/Linux-NetSpeed/master/bbr/centos/tcp_nanqinlang.c
		...
		mkdir bbrmod && cd bbrmod
		wget -N --no-check-certificate https://gh-proxy.com/raw.githubusercontent.com/chiakge/Linux-NetSpeed/master/bbr/tcp_nanqinlang.c
		echo "obj-m := tcp_nanqinlang.o" > Makefile
		make -C /lib/modules/$(uname -r)/build M=`pwd` modules CC=/usr/bin/gcc-4.9
...
startlotserver(){
	...
	bash <(wget --no-check-certificate -qO- https://gh-proxy.com/raw.githubusercontent.com/chiakge/lotServer/master/Install.sh) install
...
remove_all(){
  ...
  sed -i '/fs.file-max/d' /etc/sysctl.conf
	...
	if [[ -e /appex/bin/lotServer.sh ]]; then
		bash <(wget --no-check-certificate -qO- https://gh-proxy.com/github.com/MoeClub/lotServer/raw/master/Install.sh) uninstall
	fi
	clear
	echo -e "${Info}:清除加速完成。"
	sleep 1s
